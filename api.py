...
async def lifespan(app: FastAPI):
    ...
    try:
        # 1. Manually initialize Hydra
        # We assume the 'configs' folder is in the root directory
        with initialize(version_base=None, config_path="configs"):
            # Load the inference configuration
            cfg = compose(config_name="config_inference")
            
            # (Optional) Override config values if needed for the API environment
            # cfg.inference.mode = "single" 
